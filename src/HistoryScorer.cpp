    ...
    for (const auto& [watched_film, rating] : user_history) {
        if (watched_film->get_genre() == film.get_genre()) {
            if (watched_film->get_director() == film.get_director()) {
                if (rating == Rating::Perfect) director_effect = EFFECT_PERFECT;
                else if (rating == Rating::Average) director_effect = EFFECT_AVERAGE;
                else if (rating == Rating::Poor) director_effect = EFFECT_POOR;
            }
            if (watched_film->get_cast() == film.get_cast()) {
                 if (rating == Rating::Perfect) cast_effect = EFFECT_PERFECT;
                 else if (rating == Rating::Average) cast_effect = EFFECT_AVERAGE; 
                 else if (rating == Rating::Poor) cast_effect = EFFECT_POOR; 
            }
        }
    }
